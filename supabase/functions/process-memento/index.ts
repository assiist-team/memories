...
interface ProcessMementoRequest {
  memoryId: string;
}

interface ProcessMementoResponse {
  title: string;
...
const FALLBACK_TITLE = "Untitled Memento";
 ...
 * Memento-specific: emphasizes significance and meaning
 */
async function processTextWithLLM(
  ...
  const prompt = `Transform this transcribed text into clean, readable text optimized for human reading. The text describes a special memento or keepsake. The text comes from voice dictation and may contain run-on sentences, incomplete thoughts, and filler words. Your task is to:
...
Return only the cleaned text, nothing else.`;
  ...
  const requestBody = {
    ...
    messages: [
      {
        role: "system",
        content: "You are a helpful assistant that processes transcribed text about mementos into clean, readable format while preserving all information and emphasizing significance.",
      },
      {
        role: "user",
        content: prompt,
 ...
 * Memento-specific: emphasizes significance
 */
async function generateTitleWithLLM(
  ...
  const prompt = `Generate a concise, engaging title (maximum ${MAX_TITLE_LENGTH} characters) for a special memento or keepsake based on this cleaned text. The title should capture the significance and meaning of this memento. Return only the title text, nothing else.
...
Text: ${processedText.substring(0, 1000)}`;
  ...
  const requestBody = {
    ...
    messages: [
      {
        role: "system",
        content: "You are a helpful assistant that generates concise, engaging titles for special mementos and keepsakes.",
      },
      {
        role: "user",
        content: prompt,
...
/**
 * Edge Function for processing mementos
 * 
 * This function:
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    let requestBody: ProcessMementoRequest;
    try {
    ...
    // Verify this is a memento
    const memoryType = memory.memory_type;
    if (memoryType !== "memento") {
      return new Response(
        JSON.stringify({
          code: "INVALID_REQUEST",
          message: "This function only processes mementos",
        } as ErrorResponse),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
    ...
    console.log(
      JSON.stringify({
        event: "memento_processing",
        userId: user.id,
        memoryId: requestBody.memoryId,
        status: status,
        titleLength: title.length,
        processedTextLength: processedText?.length ?? 0,
    ...
    const response: ProcessMementoResponse = {
      title: title!,
    ...
    console.error("Unexpected error in process-memento function:", error);
