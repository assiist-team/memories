-- Migration: Add memory_location_data to get_memory_detail function
-- Description: Includes memory_location_data field in memory detail RPC response
--              This returns memory location (where event happened) separately from captured_location
...
BEGIN
  ...
  -- Build location_data JSONB object (captured location - where phone was)
  ...
  -- Set memory_location_data (where event happened)
  v_memory_location_data := v_memory.memory_location_data;
  ...
  -- Return the result with memory_location_data
  RETURN QUERY SELECT
    v_memory.id,
    v_memory.user_id,
    v_memory.title,
...
'Returns all fields needed for memory detail view including photos, videos, location data (captured location), '
'memory_location_data (where event happened), related memories, audio fields for stories, and memory_date for user-specified date.';
